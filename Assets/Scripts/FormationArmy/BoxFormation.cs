...
public class BoxFormation : FormationBase {
    ...
    private void GetCountEnemy()
    {
        ...
        {
            GameManager.Instance.CountEnemy = _unitWidth + (_unitDepth + _unitDepth) + 1;
        } else if(_unitWidth >= _unitDepth && _hollow == true && _nthOffset == -1)
        {
            GameManager.Instance.CountEnemy = _unitDepth + (_unitWidth + _unitWidth);
        }
        else if (_unitWidth >= _unitDepth && _hollow == true && _nthOffset == 0)
        {
            GameManager.Instance.CountEnemy = _unitDepth + (_unitWidth + _unitWidth) - 1;
        }
    }
}